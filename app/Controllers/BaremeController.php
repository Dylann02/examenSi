...
class BaremeController extends BaseController
{
    ...
    {
        $selectedOperateur = $this->request->getGet('operateur');
        $selectedOperation = $this->request->getGet('operation');

        $data = [
            'baremes'            => $this->baremeModel->getBaremesWithDetails($selectedOperateur, $selectedOperation),
            'operateurs'         => $this->operateurModel->findAll(),
            'operations'         => $this->typeOpModel->findAll(),
            'selected_operateur' => $selectedOperateur,
